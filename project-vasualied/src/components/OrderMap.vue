<template>
	<div class="order-map">
		<div id="mapChart" class="order-map"></div>
	</div>
</template>
<script>
	import mapJson from '../assets/datas/map.json'
		...
		methods:{
			loadChart(){
				this.$nextTick(() => {
			        ...
			        this.myChart = echarts.init(document.getElementById('mapChart'),'westeros');
			...
			initOption(){
					...
					const convertData = function(mapJson) {
					  const res = []
					  for (let i = 0; i < mapJson.length; i++) {
					    const geoCoord = geoCoordMap[mapJson[i].name]
					    if (geoCoord) {
					      res.push({
					        name: mapJson[i].name,
					        value: geoCoord.concat(mapJson[i].value)
					      })
					    }
					  }
					  return res
					}
					const convertToLineData = function(mapJson, gps) {
					  const res = []
					  for (let i = 0; i < mapJson.length; i++) {
					    const dataItem = mapJson[i]
